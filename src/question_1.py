...
def display3D_phaseplane(i_max: float, v_min: float, v_max: float):
	m = FHNModel()
	numtick = 1000
	i, v, nullV, nullW = m.nullclines_3D(i_max, v_min, v_max, numtick)
	colorscale = [[0, 'rgb(30, 144, 255)'],
	              [1, 'rgb(255, 127, 14)']]
	...
	intersect_i, intersect_v, intersect_w = m.nullcline_intersect(np.linspace(v_min, stop=v_max, num=numtick))
	fig.add_trace(
		go.Scatter3d(
